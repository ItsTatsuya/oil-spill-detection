...
def load_dataset(data_dir='dataset', split='train', batch_size=8):
    ...
    # Calculate number of batches (no repeat = finite dataset)
    ...
    if split == 'train':
        # For training: shuffle, batch, and prefetch (no repeat = finite dataset)
        dataset = dataset.shuffle(buffer_size=min(100, num_samples))
        dataset = dataset.batch(batch_size)
        # Set options for performance optimization
        options = tf.data.Options()
        options.experimental_optimization.map_parallelization = True
        options.experimental_optimization.map_fusion = True
        dataset = dataset.with_options(options)
        # Use a small prefetch buffer to avoid memory issues (2-4 batches is usually sufficient)
        dataset = dataset.prefetch(buffer_size=2)
    else:
        # For testing: batch and prefetch (no repeat = finite dataset)
        dataset = dataset.batch(batch_size)
        dataset = dataset.prefetch(buffer_size=2)
